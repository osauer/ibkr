...
2. Publish the open MCP Registry metadata only through `make registry-publish RELEASE_VERSION=vX.Y.Z MCP_PUBLISHER=bin/mcp-publisher`. If the Registry JWT is expired, the target starts `mcp-publisher login github`, prints the device-code instructions, waits for authorization, and retries publish automatically. `make release` also runs `make release-auth-preflight` before any gate work, so an expired JWT is normally refreshed at the start of the pipeline with the operator present; the in-target retry remains as a backstop.
