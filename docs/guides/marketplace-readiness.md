...
2. Publish the open MCP Registry metadata only through `make registry-publish RELEASE_VERSION=vX.Y.Z MCP_PUBLISHER=bin/mcp-publisher`. If the Registry JWT is expired, the target starts `mcp-publisher login github`, prints the device-code instructions, waits for authorization, and retries publish automatically. Registry JWTs live only about 5 minutes, so this in-target login is the normal path on every publish, not a recovery: `make release` runs `make release-auth-preflight` before any gate work to verify gh auth and the publish tooling up front, but the device-code login itself always happens at the registry-publish leg — plan to be at a browser near the end of the pipeline.
