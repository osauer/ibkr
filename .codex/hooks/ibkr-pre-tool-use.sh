...
if has_re '(^|[[:space:]/])ibkr[[:space:]]+proposals[[:space:]]+submit([[:space:]]|$)'; then
  block "Codex cannot submit protection proposals. Preview/status only; a human must run broker-write commands."
fi
...
if has_re '(^|[[:space:]/])ibkr[[:space:]]+order[[:space:]]+(place|submit|execute)([[:space:]]|$)' ||
  ...
  block "Codex cannot place, submit, execute, or transmit broker orders. Preview/status only."
fi

if has_re '(^|[[:space:]/])ibkr[[:space:]]+order[[:space:]]+modify([[:space:]]|$)' ||
  has_re '(^|[[:space:]/])ibkr[[:space:]]+modify([[:space:]]|$)'; then
  block "Codex cannot modify broker orders. Preview/status only."
fi

if has_re '(^|[[:space:]/])ibkr[[:space:]]+order[[:space:]]+(cancel|close)([[:space:]]|$)' ||
  has_re '(^|[[:space:]/])ibkr[[:space:]]+(cancel|close)([[:space:]]|$)'; then
  block "Codex cannot cancel or close broker orders. Preview/status only."
fi

if has_re '(^|[[:space:]/])ibkr[[:space:]]+(order|orders|trading|trade|trades|submit|place|cancel|close|modify|transmit)([[:space:]]|$)'; then
  block "Broker mutation verbs are blocked for Codex. Use read-only status/open-order surfaces or preview-only order review."
fi
