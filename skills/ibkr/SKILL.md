...
- `ibkr gamma [--only=spy|spx] [--no-wait] [--force] [--json]` — combined SPY+SPX dealer zero-gamma estimate (Perfiliev convention, BS gamma summed across the 6 nearest non-0DTE expirations within ATM ±10%). The default scope is **combined**: the daemon computes both SPY (the ETF, continuous extended-hours quoting on SMART/ARCA) and SPX (the index, with both SPX-class AM-settled monthlies and SPXW-class PM-settled weeklies — AM/PM settlement is honoured in the DTE filter), and returns a top-level headline plus a per-index breakdown under `per_index.{SPY,SPX}`. **Entitlement-graceful fallback:** if the account doesn't have CBOE OPRA entitlements (or the SPX option chain is unreachable), the result drops back to SPY-only and the text renderer prints a one-line `⚠ SPX skipped — <reason>. Showing SPY only.` banner above the headline. Exit code stays 0. Partial cases — e.g. SPXW lands but SPX-class 354s — surface in `partial_classes` (map keyed by the unreachable class). `--only=spy` requests the SPY-only path (regression target, bit-for-bit identical to pre-coverage-arc output); `--only=spx` requests SPX-only (exits 1 if SPX is unreachable rather than degrading). Compute is heavy — multi-minute fan-out across hundreds of legs. The first caller of an NY trading day kicks a background job and gets `status: "computing"` with an `eta_seconds`; subsequent callers within the session receive `status: "ready"` instantly. By default the CLI blocks until ready; pass `--no-wait` for the polling shape. Two complementary signals on every ready result: the *signed* `zero_gamma` (regime hint that can invert near covered-call-ETF flow or autocall barriers) and a sign-agnostic `gamma_total_abs` + `top_strikes` view that's robust to the dealer-positioning assumption. The top-strikes table carries an `INDEX` column in combined scope (`SPY` / `SPX`) so the reader can see which book's positioning is driving the headline. **Treat the number as a regime hint, not a precise level** — surface methodology caveats when the user is about to act on it.
- `ibkr regime [--explain] [--watch [--rate 5m]] [--json]` — single-call risk-regime dashboard: all five indicators (VIX/VIX3M term structure, HYG vs SPY divergence, USD/JPY weekly move, combined SPY+SPX dealer zero-gamma, SPX breadth) in one JSON envelope. Each row carries raw measurements plus a `notes` field embedding the spec's threshold bands verbatim; green/yellow/red derivation is intentionally on the consumer side. The text view is compact by default — a header row keys five columns (state · indicator · value · band · note); pass `--explain` to expand into the full provenance view with day-N streak markers, ETA clocks on `computing` rows, methodology tokens, and the spec's threshold prose under each row. JSON (`--json`) is unaffected by `--explain` and always carries every field. Expect these per-indicator failure modes on the first call against a fresh daemon: Indicator 4 (gamma) returns `status: "computing"` with `eta_seconds` (multi-minute background compute); Indicator 5 (breadth) does the same while the 50-DMA engine bootstraps (~60 min — see breadth bullet). Indicators 1–3 may carry a `fields_missing` array listing advisory sub-fields (e.g. `spy_52w_high`, `hyg_50dma`) that didn't land in the fetch budget — the row's primary measurement still landed, so treat `fields_missing` as a render hint, not an error. Reach for `--explain` when the user asks "what does this band mean?", "how long has it been in this band?", or "what's the ETA on the computing row?". `--watch` re-polls every 5 minutes by default (regime moves on minute-to-hour scales).
