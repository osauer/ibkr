...
- **S&P 500 market breadth.** `ibkr breadth` returns the percentage of S&P 500 constituents trading above their 50-day SMA — the same metric as S&P DJI's `S5FI` — plus a trailing daily series. The daemon computes the number locally from constituent daily closes (IBKR doesn't redistribute the `S5FI` index on retail subscriptions, verified via `reqContractDetails`). A once-daily refresh post-close (16:35 ET) fans out across the 500 constituent symbols, slides each name's 50-day window forward, and persists the result; readers see a cached snapshot, never a multi-minute fan-out. Cold start (fresh daemon) returns `status: "computing"` for ~10–15 min while the engine bootstraps; subsequent calls are instant. Useful as an input to a risk-regime check; see `docs/specs/risk-regime-dashboard.md` for one such dashboard the daemon's two new endpoints (this one and the next) feed directly.
