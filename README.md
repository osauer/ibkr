...
- **Positions with live Greeks.** Each option leg carries delta, gamma, theta, vega when IBKR delivers the model-computation tick. A `portfolio` block aggregates effective delta in share-equivalents, dollar delta, daily theta, gamma, vega, plus an FX-sensitivity rollup for multi-currency books. `--by underlying` consolidates stock and option legs per name.
...
- **Option chains.** `ibkr chain SPY` lists expiries with ATM implied vol by default; `--expiry 2025-12-19` switches to the strike grid. Per-strike call/put delta surfaces on the wire. Chain IV is cached daemon-side with phase-aware TTL (60 s during RTH, 4 h otherwise) so repeated lookups within a decision pause cost zero gateway round trips.
...
- **Position sizing.** `ibkr size --symbol AAPL --entry 207.50 --stop 202.50 --risk-pct 1` does fixed-fractional math against live NLV. Pure arithmetic; never proposes or attempts an order.
