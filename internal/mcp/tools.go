...
var Tools = []Tool{
	...
	},
	{
		...
		Description: strings.Join([]string{
			...
			"The compact MCP response leads with `fingerprint` (semantic identity for the classified regime state), market-scoped `lifecycle` (`scope: \"market\"`, `stage`, `severity`, `readiness`, `timing`, `confidence`, `evidence[]`, `confirmed_by[]`, lifecycle fingerprint, and `not_execution`), `source_health[]` (per cluster `as_of`, stale/degraded/partial status, confidence, and semantic-bucket fingerprint stability), `summary`, `data_quality`, and `composite` raw + cluster counts, then the eight indicator rows.",
			"Lifecycle stages are `quiet`, `early_warning`, `confirmed_stress`, `panic`, `stabilization`, `opportunity`, or `data_quality`; isolated noisy red evidence remains visible in `lifecycle.evidence` and row bands but does not dominate the trigger unless severity, tape, or independent fresh clusters confirm it.",
			"Per-row fields include raw measurements, `status`, `band`, `band_reason`, `thresholds` (heuristic + pending_backtest), `as_of` (`label`, freshness/source/time/date), `streak`, and per-scalar `*_quality` provenance.",
			...
			"Gamma embeds the compact `ibkr_gamma` envelope with profiles stripped: `envelope.result.quality.rankability` gates whether gamma can vote.",
			"Only `rankable` gamma contributes a band, cluster count, lifecycle evidence, or `confirmed_by`; `context_only`, `blocked`, and `unavailable` gamma are data-quality/context evidence only.",
	...
	},
	{
		...
		Description: strings.Join([]string{
			...
			"Standalone portfolio limit breaches become rebalance/watch context rather than market-stress alerts; stale account or positions snapshots block dependent margin, P&L, exposure, concentration, and option signals with explicit `blocked_by` sources.",
			"Degraded or computing gamma/breadth becomes explicit input-health evidence, not a false safe/false red signal; stale/degraded/partial confirming clusters cannot upgrade `market_confirmation` to confirmed until refreshed.",
			"Works pre-market and after hours by relying on account, positions, and regime freshness/status metadata; it does not call option chains, and it refuses to escalate solely on incomplete computed surfaces.",
