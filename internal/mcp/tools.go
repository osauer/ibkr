...
var Tools = []Tool{
	...
	{
		Name:        "ibkr_regime",
		Description: "Risk-regime snapshot — single-call answer for \"how does the market regime look today?\", \"is this a risk-on or risk-off tape?\", \"are we close to any of the spec's regime-shift thresholds?\", \"give me the daily-check dashboard.\" Returns all five risk-regime dashboard indicators (VIX/VIX3M term structure, HYG vs SPY divergence, USD/JPY weekly move, combined SPY+SPX dealer gamma, S&P 500 breadth) in one JSON envelope. The top-level `composite` rollup gives `{verdict, green_count, yellow_count, red_count, ranked_count, unranked_count}` so consumers can show the headline without re-implementing band logic. Per-row fields include raw measurements, explanatory `notes`, `streak: {band, sessions, since}`, and per-scalar `*_quality: {freshness_class, confidence, source, as_of}` provenance. The gamma row embeds the same `ibkr_gamma` envelope: in combined scope there is no top-level combined zero-gamma price because SPY and SPX use different price scales; use `envelope.result.summary`, `envelope.result.per_index.SPY`, and `envelope.result.per_index.SPX` for the signed regime, and use `gamma_total_abs` / `top_strikes` for sign-agnostic magnitude. `horizon_agreement` is populated only for single-underlying gamma envelopes; combined horizon buckets live under each per-index result. Expect these failure modes: gamma may return `status: \"computing\"` with ETA on the first NY-session call; SPX breadth may return `status: \"computing\"` on a fresh daemon; optional fields can appear in `fields_missing` while the row's primary measurement still landed. The result also carries `spec_doc` pointing at the canonical methodology reference.",
		JSONSchema:  schemaObject(nil, nil),
		Handler: func(ctx context.Context, conn *dial.Conn, _ json.RawMessage) (json.RawMessage, error) {
