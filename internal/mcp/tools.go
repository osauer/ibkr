...
var Tools = []Tool{
	...
	},
	{
		...
		Description: "Live portfolio canary for scheduled checks every few minutes. Use when the user asks whether to hold, watch, significantly de-leverage, liquidate, pre-stage a defense plan, or identify constructive/opportunity pressure. Returns a compact ordered action table plus machine-readable `signals[]` with stable IDs, direction (`defensive`, `constructive`, `mixed`, `data_quality`), severity (`observe`, `watch`, `act`, `urgent`), observed values, thresholds, targets, confidence impact, and blocking degraded inputs so a future `risk-plan` can consume the trigger without parsing prose. High-precision policy: severe defensive actions require independent confirmation across market clusters or immediate account-margin danger; degraded or computing gamma/breadth becomes an explicit data-quality signal, not a false safe/false red signal. Works pre-market and after hours by relying on account, positions, and regime freshness/status metadata; it does not call option chains, and it refuses to escalate solely on incomplete computed surfaces. This tool is read-only and does NOT place, preview, submit, modify, or cancel orders. NOT for detailed diagnostics — use `ibkr_regime`, `ibkr_positions`, or `ibkr_account` when you need underlying evidence; use `ibkr_positions` for held-option warnings such as `mark_outside_bid_ask` or `options_closed`.",
		JSONSchema:  schemaObject(nil, nil),
		Handler: func(ctx context.Context, conn *dial.Conn, _ json.RawMessage) (json.RawMessage, error) {
