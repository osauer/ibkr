...
var Tools = []Tool{
	...
	{
		Name:        "ibkr_regime",
		Description: "Risk-regime snapshot — single-call, non-advisory answer for \"how does the market regime look today?\", \"is this a risk-on or risk-off tape?\", \"are we close to stress thresholds?\", or \"give me the daily-check dashboard.\" Use this when the user wants the market's current evidence balance across equity vol (VIX/VIX3M + VVIX), rates vol (MOVE), credit (HYG/SPY + official HY/IG OAS), funding stress (CP/T-bill spread), FX carry proxy, dealer gamma, and S&P 500 breadth. NOT for a trade recommendation or probability forecast. The compact MCP response leads with `summary` (`label`, cluster-level `evidence`, `indicator_evidence`, `punch_line`, `confidence`, `dominant_risks`, `not_advice`) and `composite` raw + cluster counts, then the nine raw indicator rows. `warning_details` gives scoped prose for unavailable/stale/computing rows with `{code, scope, severity, message, impact, action}`; do not parse opaque error strings when this field is present. Per-row fields include raw measurements, `status`, `streak`, and per-scalar `*_quality` provenance. Methodology prose is omitted from MCP for compactness; use `spec_doc` or CLI `ibkr regime --explain` for full threshold notes. Gamma embeds the compact `ibkr_gamma` envelope with profiles stripped: in combined scope use `envelope.result.summary`, `per_index.SPY`, `per_index.SPX`, `gamma_total_abs`, and `top_strikes`; the signed γ-zero is a regime hint, not a precise level. Expect gamma/breadth to be `computing` on cold starts and optional `fields_missing` values when a secondary scalar missed the fetch budget or an official daily file is temporarily unavailable.",
		JSONSchema:  schemaObject(nil, nil),
		Handler: func(ctx context.Context, conn *dial.Conn, _ json.RawMessage) (json.RawMessage, error) {
