...
func init() {
	commands = []Command{
		...
		{"policy", "Risk constitution: effective limits, capital/drawdown state, overrides (human-only writes)", "ibkr policy show [--explain] [--json] | ibkr policy capital-event deposit|withdrawal|reconcile [--amount F] [--effective-at TIME] [--note S] | ibkr policy override --control KEY --reason S --hours N | ibkr policy reset-drawdown --reason S | ibkr policy artefact morning|eod|weekly [--note S]", runPolicy},
