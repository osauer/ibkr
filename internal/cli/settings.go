...
func renderSettingsText(env *Env, st *rpc.PlatformSettings) {
	...
	statusRow(env, out, "Max notional", fmt.Sprintf("%.2f (%s)", st.Trading.Limits.MaxNotional.Value, accessSummary(st.Trading.Limits.MaxNotional.Access, st.Trading.Limits.MaxNotional.Source)))
	statusRow(env, out, "Max option qty", fmt.Sprintf("%d (%s)", st.Trading.Limits.MaxOptionContracts.Value, accessSummary(st.Trading.Limits.MaxOptionContracts.Access, st.Trading.Limits.MaxOptionContracts.Source)))
