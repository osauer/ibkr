...
func TestProposalRevisionIgnoresRegimeLifecycleChurn(t *testing.T) {
	...
	scope := brokerStateScope{Account: "DU3136804", Mode: rpc.AccountModePaper}
...
func TestBrokerScopeConcreteRejectsAggregateIdentities(t *testing.T) {
	...
	cases := []struct {
		...
		{"paper account", brokerStateScope{Account: "DU3136804", Mode: rpc.AccountModePaper}, true},
		...
		{"multi-account list", brokerStateScope{Account: "DU3136804,U1234567", Mode: rpc.AccountModeLive}, false},
...
func TestProposalSnapshotServeRefusesScopeMismatch(t *testing.T) {
	...
	e := newProposalScopeTestEngine(t, srv)
	e.snapshot = scopedTestSnapshot("DU3136804", rpc.AccountModePaper, now)
	...
	if e.snapshot.AccountID != "DU3136804" || len(e.snapshot.Proposals) != 1 {
	...
	e.scope = func() brokerStateScope { return brokerStateScope{Account: "du3136804", Mode: rpc.AccountModePaper} }
...
func TestProposalInstallScopedFailsClosedOnScopeChange(t *testing.T) {
	...
	e.scope = func() brokerStateScope { return brokerStateScope{Account: "U1234567", Mode: rpc.AccountModeLive} }
	snap := scopedTestSnapshot("DU3136804", rpc.AccountModePaper, now)

	got := e.installScoped(snap, brokerStateScope{Account: "DU3136804", Mode: rpc.AccountModePaper}, false)
...
func TestInstallProposalEngineFailsClosedOnLegacySnapshot(t *testing.T) {
	...
	scoped, err := json.Marshal(scopedTestSnapshot("DU3136804", rpc.AccountModePaper, now))
	if err != nil {
		t.Fatal(err)
	}
	srv = writeCurrent(t, string(scoped))
	if got := srv.tradeProposals.snapshot; !got.LoadedFromState || got.AccountID != "DU3136804" || got.AccountMode != rpc.AccountModePaper {
...
func TestProposalPreserveOnFailureDropsForeignScopeSnapshot(t *testing.T) {
	...
	preserved, ok := e.preserveSnapshotOnRefreshFailure(
		brokerStateScope{Account: "DU3136804", Mode: rpc.AccountModePaper},
		rpc.AutoTradeStatus{},
		rpc.ProtectionPolicyStatus{Status: rpc.ProtectionPolicyStatusDefault},
		[]rpc.TradingBlocker{{Code: "account_unavailable", Message: "transient"}},
		false,
...
func TestProposalIgnoreIsScopedPerAccountMode(t *testing.T) {
	...
	srv := newProposalScopeTestServer(t, discover.Endpoint{}, now)
	e := newProposalScopeTestEngine(t, srv)
	paper := brokerStateScope{Account: "DU3136804", Mode: rpc.AccountModePaper}
...
func TestProposalRevisionChangesWithScope(t *testing.T) {
	...
	proposals := []rpc.TradeProposal{{Key: "theta_hygiene:abc", Quantity: 1, PositionEffect: rpc.OrderPositionEffectClose}}
	paper := proposalRevision(policy, sources, brokerStateScope{Account: "DU3136804", Mode: rpc.AccountModePaper}, proposals)
	...
	again := proposalRevision(policy, sources, brokerStateScope{Account: "du3136804", Mode: rpc.AccountModePaper}, proposals)
	if paper != again {
...
func failedRefreshSnapshot(asOf time.Time, code string) rpc.TradeProposalSnapshot {
	snap := scopedTestSnapshot("DU3136804", rpc.AccountModePaper, asOf)
...
func TestNoteRefreshOutcomeWarnsAfterStreakAndLogsRecovery(t *testing.T) {
	...
	good := scopedTestSnapshot("DU3136804", rpc.AccountModePaper, start)
...
func TestProposalSubsystemHealthReportsRefreshStreak(t *testing.T) {
	...
	e.replaceSnapshot(scopedTestSnapshot("DU3136804", rpc.AccountModePaper, start))
	...
	e.noteRefreshOutcome(scopedTestSnapshot("DU3136804", rpc.AccountModePaper, now), nil)
...
func TestInstallSnapshotGatesJournalOnRevisionChange(t *testing.T) {
	...
	snap := scopedTestSnapshot("DU3136804", rpc.AccountModePaper, day1)
