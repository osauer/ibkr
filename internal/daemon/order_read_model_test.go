...
func TestOrdersOpenUsesPinnedConcreteAccountWhenConnectorReportsAll(t *testing.T) {
	...
	if res.Account != "DU3136804" || res.Mode != rpc.AccountModePaper || res.NotBrokerStatement == "" || len(res.Limitations) == 0 {
	...
	if status.Account != "DU3136804" || status.Mode != rpc.AccountModePaper || !status.LastLocalEventAt.Equal(now.Add(-time.Minute)) || status.NotBrokerStatement == "" || len(status.Limitations) == 0 {
