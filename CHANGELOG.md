...
- **Broker-adjacent capabilities now fail closed where the daemon lacks complete authority.** This entry supersedes older current-release wording that exempted close/reduce orders from size caps or implied that a platform toggle enabled purge/restore submission. Every equity/ETF and single-leg option order now carries the ordinary notional/quantity cap, and a sell-side apparent exit also needs short/sell-to-open permission under worst-case exposure because the daemon cannot see future manual TWS activity; a genuine blocked exit says to use TWS and reconcile. Purge/restore preview and submission are typed-disabled until exact per-leg portfolio plus account-global working-order authority exists, and option-exercise review mints no token and exposes no Canary submit action until exact option-to-underlying risk policy plus durable one-shot authority are approved. The purge/restore setting controls only its workflow/read surface. Account-base cap evidence is bound to the exact broker session from an explicit currency tag or one consistent allowlisted aggregate-value suffix; `ExchangeRate=1` and ledger rows are ineligible. Runtime trading-control updates are human-only in every mode and commit their state plus normalized-origin audit event atomically; semantic no-ops advance neither revision nor audit stream.
- **The evidence journals became queryable, and they stopped growing without bound.** The regime-decisions journal reached 53 MB in two months and could only be read with `jq` full-file scans, which is why the `pending_backtest` thresholds it was collected to calibrate had never actually been measured against it. A new derived index, `$XDG_STATE_HOME/ibkr/history.db`, mirrors seven journals plus the retained Flex statements into queryable tables, using a pure-Go SQLite driver so CGO stays off and the toolchain-free release cross-compile matrix is untouched (about 4 MB of binary). Every row carries its journal line verbatim, so a calibration question the schema does not project stays answerable with `json_extract` instead of a schema migration. The journals remain the evidence of record and the index is strictly derived: `rm history.db*` is always safe, and the daemon rebuilds from files at start — exercised on the live state directory, where the deleted 133 MB index came back with every table's row count matching its journal's line count exactly. New read surfaces `ibkr regime history`, `ibkr rules history`, `ibkr canary history`, and `ibkr recon equity` ship as subverbs of existing commands, so no MCP tool and no top-level command changed. Statement-derived daily equity now outlives the runtime 45-day sample prune (64 days on the first backfill, where the state file could only remember 45), and a new `canary-decisions.jsonl` collects the portfolio side of the canary the way the regime journal already collected the market side: daemon-owned, unredacted, and deduplicated on the canary fingerprint with an hourly heartbeat, unlike the phone's deliberately redacted 14-day alert list.
- **Decision journals now rotate into compressed archives instead of growing forever.** A daily maintenance pass moves each decision journal's fully-ingested prefix older than the keep window into `rotated/<journal>-YYYY-MM.jsonl.gz`. Rotation compresses and relocates evidence and never deletes it: only bytes already mirrored into the index are eligible, archives are immutable, and archives in file-name order concatenated with the live journal reproduce the original byte stream exactly, which is also what lets a full rebuild reproduce identical rows. Only the three decision journals (regime, rules, canary) are rotatable; the order journal, broker statements, capital and risk-policy journals, and both proposal journals are never rotated, and that list is pinned in `docs/design/history-index.md` rather than left to the code. Three runtime settings govern it: `history.rotation.enabled` (default true), `history.rotation.keep_raw_months` (default 2, minimum 1), and `canary.journal.enabled` (default true). A rotation interrupted mid-flight resolves on the next start to exactly one of rollback or roll-forward, and an interrupted rotation that leaves the daemon running now heals its own indexing on the next pass rather than stalling until restart.
- **The daily brief is now a trading-process artifact on its own tab.** Re-conceived from five data-domain sections (Market, Calendar, Portfolio, Risk & limits, Process) into two process movements the daemon composes and every surface renders verbatim: **Review** — post-trade of the last completed session (session P&L, daily attribution by underlying, process coherence [the rulebook-adherence delta plus a new proposals-offered-vs-acted count read read-only from the trade-proposal-outcomes journal, and overrides used], capital events, the reconcile clock with its one-tap sign-off, and the working-orders end state) — and **Ready** — pre-trade for today (overnight & market, calendar, risk capacity, and desk readiness). It is a regrouping of facts the daemon already had — row severities and the worst-child rollup are unchanged — with the one new derivation carrying only counts and the covered day to the wire (no proposal keys, symbols, order refs, or tokens). The brief moves off the Monitor tab to its own bottom-tab slot, "Brief" (Monitor · Brief · Alerts · Orders · Settings, Monitor still the default landing and universal fallback), with sunrise (Ready) and history-clock (Review) phase glyphs and sentence-case titles. Process/governance push nudges that used to land on Monitor now land on the Brief tab; the one-tap reconcile sign-off keeps its exact endpoint, evidence, and semantics. VaR and any risk-unit measure stay out of scope (an open operator decision).
...
- Order reads may now be served from the history index, but only while it provably matches the journal. `orders.open`, `orders.history`, preview-token redemption, and the reserved-order-id floor each re-read the whole order journal today, on a file that only grows and is never rotated. They now prefer indexed reads when the on-disk journal size equals the committed ingest watermark and no unparseable line is present, and fall back automatically to the unchanged journal scan on staleness, any query or decode error, or a parse marker, disclosing the fallback in the log. The journal scan stays the semantics-defining reference implementation: SQL only narrows the candidate set while the existing Go predicates decide, a permanent dual-read parity test in `make test` deep-compares both paths, and token redemption keeps its exact critical section, accept/reject decisions, and byte-identical error text. Nothing in broker-write behavior, submit eligibility, freeze, trading gates, or journaling semantics changed, and there is deliberately no settings escape hatch, because the fallback path is the unmodified legacy code and is exercised on every cold start.
- The public architecture page was rewritten for readers rather than converters. `docs/architecture.md` is restructured into ten numbered sections in plain English with a new observability section, and two claims were corrected against the code: HyperServe serves only the app process (the daemon speaks newline-delimited JSON over its Unix socket and MCP is stdio-only, with no HTTP transport), and the embedded exchange calendars cover 2026 through 2028 and report an explicit unknown state outside that range rather than falling back to weekday logic. The published twin at osauer.dev/ibkr/architecture.html is now a designed page with numbered chip navigation, process and ownership cards, and scroll-contained tables, and both diagrams are regenerated from the deterministic generator with tiles for the history index and the rotated archives.
