...
- `ibkr regime` gamma row label now reflects the actual scope of the underlying envelope (`SPY γ-zero` / `SPX γ-zero` / `γ-zero (SPY+SPX)`) instead of always reading `SPY γ-zero` even when the data is combined SPY+SPX.
