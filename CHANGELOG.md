...
- A single corrupted equity observation (for example during a gateway-reconnect window) could silently and permanently ratchet the risk-capital adjusted peak, overstating drawdown and consumed risk against a phantom peak until a human re-base; the trader-panel review caught exactly this state rendering as "OK". Every adjusted-peak ratchet is now journaled ("adjusted_peak_advanced" with before/after peak, the observed equity, and its timestamp), so a poisoned peak is diagnosable from the risk-policy journal alone, and the brief's value-derived statuses make the resulting state impossible to render as healthy. An observation plausibility gate and any state repair remain explicit operator decisions, deliberately outside this change.
