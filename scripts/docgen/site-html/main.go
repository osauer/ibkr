...
func renderFile(markdownPath string) ([]byte, error) {
	...
	description := documentDescription(source)
	canonical := "https://osauer.dev/ibkr/" + canonicalPath(markdownPath)
	...
	if description != "" {
		...
		out.WriteString(html.EscapeString(description))
		out.WriteString("\">\n")
	}
	out.WriteString("  <link rel=\"canonical\" href=\"https://osauer.dev/ibkr/")
	out.WriteString(html.EscapeString(strings.TrimPrefix(canonical, "https://osauer.dev/ibkr/")))
	out.WriteString("\">\n")
	writeArticleJSONLD(&out, title, description, canonical, documentTimestamp(source))
...
func writeArticleJSONLD(out *strings.Builder, title, description, canonical, modified string) {
...
func canonicalPath(markdownPath string) string {
	htmlPath := strings.TrimSuffix(markdownPath, filepath.Ext(markdownPath)) + ".html"
	canon, err := filepath.Rel(docsRoot, htmlPath)
	if err != nil {
		return filepath.ToSlash(htmlPath)
	}
	out := filepath.ToSlash(canon)
	if out == "index.html" {
		return ""
	}
...
func schemaTimestamp(text string) string {
	...
	if _, err := time.Parse("2006-01-02 15:04", fields[0]+" "+fields[1]); err != nil {
		return fields[0]
	}
	return fields[0] + "T" + fields[1] + ":00" + offset
}

func documentDescription(source string) string {
	...
	for i := 0; i < len(lines); {
		...
		if description := truncateDescription(cleanMetaText(strings.Join(paragraph, " "))); description != "" {
			return description
		}
	}
	return ""
...
func truncateDescription(text string) string {
	...
	for word := range strings.FieldsSeq(text) {
		if out.Len() == 0 {
			if utf8.RuneCountInString(word) > maxRunes-3 {
				runes := []rune(word)
		...
		if utf8.RuneCountInString(out.String())+1+utf8.RuneCountInString(word) > maxRunes-3 {
			break
		}
		out.WriteByte(' ')
		out.WriteString(word)
