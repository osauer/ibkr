...
func (c *Connector) FetchContractDetails(symbol string, timeout time.Duration) ([]ContractDetailsLite, error) {
	...
	for {
		...
		case <-deadline:
			...
			return results, fmt.Errorf("timeout waiting for contract details")
		}
	}
}
...
func (c *Connector) Stop() error {
	...
	// Release lease if we have one
	if c.lease != nil {
		if err := c.pool.ReleaseLease(c.lease.LeaseID); err != nil {
...
// look up the cached quote in GetMarketData.
func (c *Connector) SubscribeOption(underlying, expiryYMD string, strike float64, right string) (string, int, error) {
	if !c.isConnected() {
	...
	if err := conn.resolveOptionContract(&contract, 5*time.Second); err != nil {
		return "", 0, fmt.Errorf("resolve option %s %s %.2f%s: %w",
			contract.Symbol, contract.Expiry, contract.Strike, contract.Right, err)
	}

	reqID, err := conn.RequestMarketDataWithContract(contract, "", false, false)
	if err != nil {
		return "", 0, err
	}
...
func (c *Connector) handleTickPrice(fields []string) {
	...
	// Only update with validated positive prices
	switch tickType {
	case 1:
		sub.Bid = price
	case 2:
...
func (c *Connector) handleOptionComputation(fields []string) {
	...
	// Store greeks for potential diagnostics
	if !math.IsNaN(gamma) && gamma != 0 {
		// reuse optQuoteAsk map to avoid new map? better to extend struct? For now ignore.
	}
	if !math.IsNaN(vega) {
		_ = vega // placeholder for future instrumentation
	}
	if !math.IsNaN(theta) {
		_ = theta
	}
	if !math.IsNaN(underlyingPrice) && underlyingPrice > 0 {
		// We currently do not persist underlying price from option computations,
		// but keeping this branch allows easy instrumentation later.
	}
...
// expiry should be in UTC; right is "C" or "P".
func (c *Connector) SubscribeOptionIV(symbol string, expiry time.Time, strike float64, right string) (int, error) {
	symbol = strings.ToUpper(symbol)
	if _, inactive := c.inactiveReason(symbol); inactive {
	...
	reqID, err := conn.RequestOptionsMarketData(symbol, expStr, strike, strings.ToUpper(right))
	if err != nil {
		return 0, err
	}
